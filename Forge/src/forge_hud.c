...
void forge_hud_draw(Forge *f)
{
    ...
    /* WRITTEN BY HAND, and kept in step with Forge/src/forge_binds.def by hand.
     * That file is the one statement of the control scheme, but only
     * tools/forge/gen_binds.py reads it, and what that emits is the Markdown
     * table in the n64-forge skill -- nothing #includes it, and nothing compares
     * these strings with it. (This comment used to say they were "built from"
     * it, alongside a B() macro nothing used; both implied a generation that
     * never existed.) The strings once said "Z+A fill", which has never been
     * true: forge_geo.c anchors on Z press and fills on Z release, and
     * suppresses A for the whole drag.
     *
